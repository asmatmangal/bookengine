 ...
 * All AI calls go through Render proxy — no key in frontend
 */
'use strict';

/* ═══════════════════════════════════════════════
   CORE AI CALL — via Render Proxy
═══════════════════════════════════════════════ */
...
const BOOK_DATABASE = {
  ...
  romance:   [{title:'Pride and Prejudice',author:'Jane Austen',genre:'Romance',reason:'The greatest love story ever written — witty, timeless, and perfect.'},{title:'The Notebook',author:'Nicholas Sparks',genre:'Romance',reason:'A beautiful love story that will make you cry and smile at the same time.'},{title:'Outlander',author:'Diana Gabaldon',genre:'Romance',reason:'A woman travels back in time to 18th century Scotland and finds love.'},{title:'Me Before You',author:'Jojo Moyes',genre:'Romance',reason:'A life-changing love story that will break your heart beautifully.'},{title:'The Hating Game',author:'Sally Thorne',genre:'Romance',reason:'Two office rivals who secretly love each other — funny and addictive.'},{title:'It Ends with Us',author:'Colleen Hoover',genre:'Romance',reason:'A powerful emotional love story that deals with real, difficult themes.'}],
  fantasy:   [{title:'The Name of the Wind',author:'Patrick Rothfuss',genre:'Fantasy',reason:'The most beautifully written fantasy novel of the last 20 years.'},{title:'The Way of Kings',author:'Brandon Sanderson',genre:'Fantasy',reason:'An epic world-building masterpiece with an incredible magic system.'},{title:'The Lies of Locke Lamora',author:'Scott Lynch',genre:'Fantasy',reason:'Ocean\'s Eleven in a fantasy world — clever, dark, and addictive.'},{title:'The Shadow of the Wind',author:'Carlos Ruiz Zafon',genre:'Fantasy',reason:'A boy finds a mysterious book in Barcelona and enters a world of secrets.'},{title:'American Gods',author:'Neil Gaiman',genre:'Fantasy',reason:'Old gods vs new gods in America — dark, mythical, and unforgettable.'},{title:'The Night Circus',author:'Erin Morgenstern',genre:'Fantasy',reason:'A magical circus that only appears at night — visually stunning.'}],
  scifi:     [{title:'Dune',author:'Frank Herbert',genre:'Sci-Fi',reason:'The greatest science fiction novel ever written — epic and visionary.'},{title:'The Hitchhiker\'s Guide to the Galaxy',author:'Douglas Adams',genre:'Sci-Fi',reason:'The funniest book in the universe. 42. That is all.'},{title:'Ender\'s Game',author:'Orson Scott Card',genre:'Sci-Fi',reason:'A child genius trained to fight an alien war — stunning and heartbreaking.'},{title:'Project Hail Mary',author:'Andy Weir',genre:'Sci-Fi',reason:'A man wakes alone in space with no memory — pure science-driven survival.'},{title:'The Left Hand of Darkness',author:'Ursula K. Le Guin',genre:'Sci-Fi',reason:'A world with no gender — one of the most thought-provoking books ever.'},{title:'Flowers for Algernon',author:'Daniel Keyes',genre:'Sci-Fi',reason:'A man with low IQ becomes a genius — told through diary entries.'}],
  biography: [{title:'Steve Jobs',author:'Walter Isaacson',genre:'Biography',reason:'The full, unfiltered story of the most creative genius of our time.'},{title:'Becoming',author:'Michelle Obama',genre:'Biography',reason:'Michelle Obama\'s story from Chicago to the White House — honest and inspiring.'},{title:'Long Walk to Freedom',author:'Nelson Mandela',genre:'Biography',reason:'How one man\'s courage changed an entire country.'},{title:'Open',author:'Andre Agassi',genre:'Biography',reason:'The most honest sports autobiography ever written.'},{title:'Leonardo da Vinci',author:'Walter Isaacson',genre:'Biography',reason:'The full story of history\'s most curious and brilliant mind.'},{title:'Educated',author:'Tara Westover',genre:'Biography',reason:'A girl who never went to school earns a PhD from Cambridge — extraordinary.'}]
};

/* ═══════════════════════════════════════════════
   AI RECOMMENDATIONS — reads full sentence
═══════════════════════════════════════════════ */
...
IMPORTANT: Read the user's FULL request carefully. Consider every word — their mood, preferred genre, themes, authors they like, and any specific requirements.

Return ONLY a valid JSON array of exactly 6 book recommendations.
No markdown, no code blocks, no explanation — raw JSON array starting with [

Each book object must have exactly these fields:
- title: book title
- author: author full name
- genre: genre
- reason: 1-2 warm specific sentences explaining WHY this matches what they asked`;

    const prompt = `The reader says: "${prefs}"

Carefully read their full request and recommend exactly 6 books that perfectly match everything they described. Return only the JSON array.`;
...
function smartFallbackRecs(prefs, container) {
  ...
  const scores = {
    romance:   scoreKw(kw, ['romance','love','relationship','emotional','couple','heart','wedding','kiss']),
    thriller:  scoreKw(kw, ['thriller','suspense','mystery','dark','scary','horror','psychological','twist']),
    history:   scoreKw(kw, ['history','historical','war','ancient','past','century','civilization','empire']),
    space:     scoreKw(kw, ['space','cosmos','universe','planet','astronaut','galaxy','nasa','stars']),
    scifi:     scoreKw(kw, ['sci-fi','science fiction','future','robot','ai','dystopia','cyberpunk','alien']),
    fantasy:   scoreKw(kw, ['fantasy','magic','wizard','dragon','sword','kingdom','mythical','adventure']),
    crime:     scoreKw(kw, ['crime','detective','murder','police','investigation','serial killer','heist']),
    selfcare:  scoreKw(kw, ['self help','motivat','habit','productivity','wellness','mental health','healing']),
    tech:      scoreKw(kw, ['tech','startup','coding','entrepreneur','silicon valley','business','innovation']),
    biography: scoreKw(kw, ['biography','memoir','true story','autobiography','real life','celebrity','leader']),
    nature:    scoreKw(kw, ['nature','wildlife','animals','plants','forest','ocean','environment','outdoor']),
  };
  const best = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0];
  renderAIRecs(BOOK_DATABASE[best] || BOOK_DATABASE.thriller, container);
  showToast('Showing curated picks based on your request!', 'info');
}

function scoreKw(text, keywords) {
  return keywords.reduce((s, k) => s + (text.includes(k) ? 1 : 0), 0);
}
...
async function fetchAISummary(bookId, title, description) {
  ...
  try {
    ...
    let   prompt = 'Write a short exciting 3-sentence summary of "' + title + '".';
    if (description && description.length > 20) prompt += ' Info: ' + description.replace(/<[^>]*>/g, '').substring(0, 400);
  ...
  if (description && description.length > 30) {
    ...
    box.innerHTML = '<div class="ai-summary-box"><div class="ai-label"><i class="fas fa-sparkles"></i> AI Summary</div><p style="color:var(--text-muted);">No summary available for this book.</p></div>';
  }
}
    ...
    <div id="chatWidget" class="chat-widget hidden">
      ...
      <div class="chat-messages" id="chatMessages">
        <div class="chat-msg ai">
          <div class="chat-bubble">Hey! 👋 I'm your <strong>Book AI</strong>. Ask me anything — recommendations, summaries, authors, or just tell me your mood and I'll find the perfect book!</div>
        </div>
...
async function sendChatMessage() {
  ...
  try {
    const sys = `You are Book AI — a friendly, enthusiastic, knowledgeable book expert.
Talk like a smart friend who genuinely loves books. Use simple, clear English.
When recommending books give 3-4 suggestions in this format:
**Book Title** by Author — one sentence why they will love it.
Keep responses under 200 words. Be warm, encouraging, and specific.
Remember the full conversation and refer back to what the user said earlier.`;
    ...
    document.getElementById(typingId)?.remove();
    appendChatMessage('ai', '📚 Sorry, I had a quick hiccup! Please ask me again.');
  }
}
